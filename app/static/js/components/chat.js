...
function setUserResponse(message) {
    const user_response = `<img class="userAvatar" src='../static/img/userAva.png'><p class="userMsg">${message} </p><div class="clearfix"></div>`;
...
function getBotResponse(text) {
    botResponse = `<img class="botAvatar" src="../static/img/bot_img.png"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;
    return botResponse;
}
...
function setBotResponse(response) {
    ...
    setTimeout(() => {
        ...
        if (response.length < 1) {
            ...
            const fallbackMsg = "Xin lỗi, hiện tại không thể kết nối với server, vui lòng quay lại sau!!!";

            const BotResponse = `<img class="botAvatar" src="../static/img/bot_img.png"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
            ...
            for (let i = 0; i < response.length; i += 1) {
                ...
                if (Object.hasOwnProperty.call(response[i], "text")) {
                    if (response[i].text != null) {
                        ...
                        if (html.includes("<ul") || html.includes("<ol") || html.includes("<li") || html.includes('<h3')) {
                            ...
                            if (!botResponse) {
                                botResponse = `<img class="botAvatar" src="../static/img/bot_img.png"/><p class="botMsg">${response[i].text}</p><div class="clearfix"></div>`;
                            }
                        }
                ...
                if (Object.hasOwnProperty.call(response[i], "custom")) {
                    ...
                    //check of the custom payload type is list
                    if (payload === "list_show") {
                        const {data} = response[i].custom;
                        createList(data);
                    }
                }
            ...
            let s1 = ls[0];
            if (chk_audio) {
                if (s1.indexOf("Tên thủ tục:") > -1) {
                    responsiveVoice.speak("Thủ tục bạn cần tìm có thông tin như sau: ", 'Vietnamese Female');
                } else {
                    responsiveVoice.speak(s, 'Vietnamese Female');
                }
            }
